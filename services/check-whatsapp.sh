#!/usr/bin/env bash
# Check WhatsApp Web for new messages every few minutes via the OpenBot computer
# browser (which holds the linked WhatsApp session for the whatsapp-assistant
...
STATE_FILE="$HOME/.local/state/openbot-whatsapp-state"
LOG_FILE="$HOME/.local/state/openbot-whatsapp-check.log"
...
if ! dockerx 'docker ps -q -f name=^alexander-computer-whatsapp-assistant$' | grep -q .; then
  exit 0
fi

TOKEN="$(dockerx 'docker exec alexander-computer-whatsapp-assistant sh -c "echo $COMPUTER_TOKEN"' | tr -d '\r\n')"
[ -n "$TOKEN" ] || exit 0
...
H2="x-openbot-bot-id: whatsapp-assistant"
CT="Content-Type: application/json"
curl -s -m 90 -X POST -H "$H1" -H "$H2" -H "$CT" \
  -d '{"url":"https://web.whatsapp.com/"}' \
  http://127.0.0.1:4100/navigate >/dev/null
sleep 4
curl -s -m 90 -H "$H1" -H "$H2" http://127.0.0.1:4100/read
EOF
dockerx "docker cp $REMOTE alexander-computer-whatsapp-assistant:/tmp/openbot-whatsapp-check.sh" >/dev/null
TEXT="$(dockerx "docker exec -e CHECK_TOKEN=$TOKEN alexander-computer-whatsapp-assistant sh /tmp/openbot-whatsapp-check.sh")"
dockerx 'docker exec alexander-computer-whatsapp-assistant rm -f /tmp/openbot-whatsapp-check.sh' >/dev/null
...
if command -v notify-send >/dev/null 2>&1; then
  notify-send -a "WhatsApp check" "New WhatsApp activity" "$SUMMARY" 2>/dev/null || true
fi
