#!/usr/bin/env bash
# Check Facebook Messenger for new messages every few minutes via the OpenBot computer
# browser (which holds the linked WhatsApp session for the messenger-assistant
...
BOT="messenger-assistant"
STATE_FILE="$HOME/.local/state/openbot-messenger-state"
LOG_FILE="$HOME/.local/state/openbot-messenger-check.log"
...
if ! dockerx 'docker ps -q -f name=^alexander-computer-messenger-assistant$' | grep -q .; then
  exit 0
fi

TOKEN="$(dockerx 'docker exec alexander-computer-messenger-assistant sh -c "echo $COMPUTER_TOKEN"' | tr -d '\r\n')"
[ -n "$TOKEN" ] || exit 0
...
H2="x-openbot-bot-id: messenger-assistant"
CT="Content-Type: application/json"
curl -s -m 90 -X POST -H "$H1" -H "$H2" -H "$CT" \
  -d '{"url":"https://www.messenger.com/"}' \
  http://127.0.0.1:4100/navigate >/dev/null
sleep 4
curl -s -m 90 -H "$H1" -H "$H2" http://127.0.0.1:4100/read
EOF
dockerx "docker cp $REMOTE alexander-computer-messenger-assistant:/tmp/openbot-messenger-check.sh" >/dev/null
TEXT="$(dockerx "docker exec -e CHECK_TOKEN=$TOKEN alexander-computer-messenger-assistant sh /tmp/openbot-messenger-check.sh")"
dockerx 'docker exec alexander-computer-messenger-assistant rm -f /tmp/openbot-messenger-check.sh' >/dev/null
...
if command -v notify-send >/dev/null 2>&1; then
  notify-send -a "Messenger check" "New Messenger activity" "$SUMMARY" 2>/dev/null || true
fi
